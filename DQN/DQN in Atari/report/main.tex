...
\begin{figure}[H]
    ...
    \caption{n\_step $\in \{1,2,3,5\}$ 的全面对比（蓝=n1，红=n2，绿=n3，橙=n5）。第一行：奖励曲线对比及收敛里程碑柱状图；第二行：TD-Loss曲线、奖励标准差、训练速度；第三行：Breakout与SpaceInvaders的训练曲线及跨环境得分对比。}
...
训练超参数与Pong保持一致（n\_step=1，lr=$10^{-4}$，20M总步数），训练曲线见图\ref{fig:nstep_comparison}第三行。
