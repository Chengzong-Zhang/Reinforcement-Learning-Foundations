...
\subsection{n-step TD目标的偏差-方差权衡与异策略偏差}

标准1-step TD目标 $G^{(1)} = r_{t+1} + \gamma V(s_{t+1})$ 的偏差来自 $V(s_{t+1})$ 的估计误差；$n$-step目标展开为：
$$G^{(n)}_t = \sum_{k=0}^{n-1} \gamma^k r_{t+k+1} + \gamma^n V(s_{t+n})$$
随着 $n$ 增大，对 $V$ 的依赖减小（Bootstrap偏差降低），奖励信号向前传播更充分，有利于加速早期学习。

然而，DQN 是 \textbf{Off-Policy（异策略）}算法，这引入了经典偏差-方差分析之外的第二类误差——\textbf{异策略偏差（Off-Policy Discrepancy）}。上述 $n$-step 目标隐式假设中间的 $n-1$ 个动作 $a_{t+1},\ldots,a_{t+n-1}$ 由当前贪心目标策略 $\pi_{\mathrm{target}} = \arg\max Q$ 执行。但 Replay Buffer 中的轨迹由过去的旧策略 $\pi_{\mathrm{old}}$ 生成，两者的动作分布存在系统性偏差。随着 $n$ 的增大，旧轨迹与当前最优路径一致的概率呈\textbf{指数级衰减}，导致 $n$-step TD 目标本身存在系统性偏差，且随着训练推进、策略快速演化而愈发显著。

因此，n-step 在 DQN 中的调优本质上是 \textbf{Bootstrap 偏差}（随 $n$ 增大而降低）与 \textbf{Off-Policy 偏差}（随 $n$ 增大而升高）之间的权衡：早期 Q 网络极不准确，Bootstrap 偏差占主导，长步展开收益显著；训练后期策略趋于收敛，Off-Policy 偏差成为主要瓶颈，过大的 $n$ 反而破坏 Bellman 方程的准确性。DeepMind 在 Rainbow DQN（2018）中通过穷举实验得出 $n=3$ 是 Atari 任务上两类偏差的最优平衡点，与本实验结论（$n=2$ 最终最稳定）逻辑自洽。
...
\paragraph{方向一：收敛速度}
\begin{itemize}
    \item \textbf{n=1} 的Bootstrap目标 $r + \gamma V(s')$ 完全依赖当前Q网络对下一状态的估计。初期网络极不精确，Bootstrap偏差极大，学习信号几乎无效，导致收敛最慢（9.1M步才首次得分$\geq0$）。
    \item \textbf{多步TD（n=2,3,5）} 的目标直接展开了更多真实奖励，有效降低了 Bootstrap 偏差，加速了早期奖励信号的回传。
    \item \textbf{n=5早期收敛最快}：首次得分$\geq0$仅需1.5M步，$\geq19$和$\geq21$也最快（分别4.2M和5.4M步）。原因在于早期Q网络估值极不准确，此时长步展开能有效传递真实奖励信号，Bootstrap偏差降低带来的收益远大于 Off-Policy 偏差的负面影响。
    \item \textbf{n=2在得分$\geq15$时最快（2.6M步）}：此阶段策略已部分收敛，Off-Policy 偏差开始显现，n=5 的旧轨迹与当前策略的偏差逐渐累积，而 n=2 凭借极低的 Off-Policy 偏差在中期率先突破。
\end{itemize}

\paragraph{方向二：训练动态}
\begin{itemize}
    \item \textbf{TD-Loss}：四者在对数尺度下整体均从 $10^{-1}$ 降至 $10^{-2}$ 量级。n=1的loss曲线中后期波动最剧烈；n=2/3/5的loss下降相对更平滑。n=5后期loss偏高，根本原因是 Replay Buffer 中旧轨迹与当前贪心策略的动作分布偏差随训练推进而持续扩大，引入系统性的目标偏差。
    \item \textbf{奖励标准差}：收敛后n=2的std最低，策略最稳定；n=5收敛后std略高，反映了后期显著的 Off-Policy 偏差对策略一致性的干扰——网络被持续注入"来自旧策略、偏离当前最优路径"的轨迹，导致Q值被错误更新，引发策略轻微震荡。
    \item \textbf{最终得分}：n=2略高（20.96），n=5紧随其后（20.91），两者均显著优于n=1（20.38）。n=5后期被n=2反超，正是训练后期 Off-Policy 偏差主导效应的体现。
\end{itemize}
...
评估维度 & 最优值 & 原因 \\
\midrule
最快收敛至$\geq0/\geq19/\geq21$ & n=5 & 早期Bootstrap偏差主导，长步展开大幅降低偏差 \\
最快收敛至$\geq15$ & n=2 & 后期Off-Policy偏差显现，n=2偏差最低 \\
最终性能 & n=2（20.96） & Off-Policy偏差最小，Bellman目标最准确 \\
训练稳定性 & n=2 & Loss曲线最平滑，Off-Policy误差累积最少 \\
\textbf{综合建议} & \textbf{n=2或n=3} & 与Rainbow DQN结论（n=3最优）逻辑一致 \\
\bottomrule
\end{tabular}
\end{center}
