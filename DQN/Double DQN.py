...
class DQN:
    ...
    def __init__(self,
                 ...
                 target_update,
                 device,
                 dqn_type='VanillaDQN'):  # dqn_type 区分普通DQN与Double DQN，默认为普通DQN
        self.action_dim = action_dim      # 保存动作空间维度，供 epsilon-贪婪策略随机选动作时使用
        self.q_net = Qnet(state_dim, hidden_dim, self.action_dim).to(device)          # 创建在线 Q 网络（用于选动作和计算当前 Q 值），并移至指定设备（CPU/GPU）
        self.target_q_net = Qnet(state_dim, hidden_dim, self.action_dim).to(device)  # 目标网络：结构与在线网络相同，但参数更新滞后，用于计算更稳定的 TD 目标
    ...
    def update(self, transition_dict):
        ...
        if self.dqn_type == 'DoubleDQN':  # Double DQN 的目标值计算方式
            ...
            max_next_q_values = self.target_q_net(next_states).gather(1, max_action)      # 用目标网络评估该动作的 Q 值（"评估"步骤，解耦选择与评估，有效降低过估计偏差）
...
target_update = 50   # 每训练50次（调用50次 update）才将在线网络参数同步到目标网络（Pendulum任务目标网络更新较慢）
...
# ===================== 环境初始化 =====================
env_name = 'Pendulum-v1'                    # Pendulum-v1：控制钟摆保持直立，原生为连续动作空间，这里将其离散化（v0已弃用，v1为新版）
        ...
        with tqdm(total=int(num_episodes / 10), desc='Iteration %d' % i) as pbar:  # 每轮创建一个进度条，显示本轮进度
            for i_episode in range(int(num_episodes / 10)):  # 每轮训练 num_episodes/10 个 episode
                ...
                while not done:             # 在 episode 未结束前持续与环境交互
                    ...
                    next_state, reward, terminated, truncated, _ = env.step([action_continuous])  # 执行连续动作，获取下一状态、奖励、是否自然终止、是否超时截断（gymnasium 返回5个值）
...
agent = DQN(state_dim, hidden_dim, action_dim, lr, gamma, epsilon,
            target_update, device, dqn_type='DoubleDQN')  # 创建 Double DQN 智能体（指定 dqn_type='DoubleDQN' 以启用解耦选择-评估机制）
...
plt.title('Double DQN on {}'.format(env_name))   # 图标题
plt.show()                                       # 显示图像
...
plt.axhline(10, c='red', ls='--')                # 红色虚线标注 Q=10（参考基准线，可对比普通DQN的过估计程度）
