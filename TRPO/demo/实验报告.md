...
本地快速验证中，脚本已通过 1 episode 烟测，可正常完成采样、TRPO 更新和图片保存：

```text
Training finished on cuda.
Last 10 episode mean return: 21.000
```
...
本地快速验证中，脚本已通过 1 episode 烟测，可正常完成连续动作采样、TRPO 更新和图片保存：

```text
Training finished on cuda.
Last 10 episode mean return: -1768.778
```
